...
export default function CandidateDetailPage() {
  ...
  const getAuditLogs = () => {
    ...
    if (candidate?.payments?.length > 0 && candidate.payments[0].status === "completed") {
      logs.push({
    ...
    if (candidate?.mobileVerified || candidate?.emailVerified) {
      logs.push({
        ...
        subtitle: "Mobile and email verification completed",
  ...
  const stepData = candidate.stepData || {};
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4">
                    {documents.map((doc) => (
                      <div key={doc.id} className="rounded border border-[#d5dcdc] bg-[#f8f9f8] p-3">
                        ...
                        <p className="text-center text-[10px] text-[#7d8782] mt-1">{doc.fileName}</p>
                        {doc.signedUrl && (
                          <a
                            href={doc.signedUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="mt-2 flex items-center justify-center gap-1 text-[11px] text-[#2c6e57] hover:underline"
                          >
              ...
              {payments.length > 0 && (
                <div className="rounded border border-[#0f6a4c] bg-[#0d6c4b] p-5 text-white shadow-sm">
                  ...
                  <h2 className="mt-3 text-[36px] font-bold font-mono">
                    ₹{parseFloat(payments[0].amount).toLocaleString('en-IN')}
                  </h2>
