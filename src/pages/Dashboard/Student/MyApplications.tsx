...
const MyApplications: React.FC = () => {
    ...
    return (
      <div className={`relative ${className}`}>
        <div
          ...
          className={`w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary cursor-pointer flex justify-between items-center bg-white`}
        >
  ...
  const MultiSelectDropdown: React.FC<{
    ...
    placeholder: string;
    error?: string;
  }> = ({ options, values, onChange, placeholder, error }) => {
    const [isOpen, setIsOpen] = useState(false);
    const [searchTerm, setSearchTerm] = useState("");
    ...
    return (
      <div className="relative">
        <div
          onClick={() => setIsOpen(!isOpen)}
          className={`min-h-[42px] w-full px-3 py-2 border rounded-lg cursor-pointer bg-white flex flex-wrap gap-2 items-center ${error ? "border-red-500" : "border-slate-300"}`}
        >
          {values.length > 0 ? (
            values.map((item) => (
              <span
              ...
              <div className={`space-y-3 ${errors.postRankings ? 'border-2 border-red-500 rounded-xl p-3' : ''}`}>
                {postsToShow.map((post, index) => {
                  ...
                  return (
                    <div
                      key={post.postCode}
                      className={`flex items-center gap-4 bg-white border rounded-lg p-3 md:p-4 shadow-sm hover:shadow-md transition-shadow ${isEligible ? 'border-green-300 bg-green-50/30' : 'border-slate-200'
                        }`}
                    >
                      <div className="flex items-center gap-3 shrink-0">
                      ...
                      <div className="shrink-0 ml-2">
                        <select
                          ...
                          className={`w-32 h-12 border rounded-lg text-center font-bold text-primary focus:border-primary outline-none px-2 ${currentPriority === 0 ? 'border-red-300 bg-red-50' : 'border-slate-300'
                            }`}
                        >
                          <option value={0}>Select Priority</option>
                          {availablePriorities.map((priority) => (
  ...
  const renderFeePayment = () => {
    ...
    if (feeCat === "exserviceman") {
      applicableFeeText = "Ex-Servicemen (Fee: ₹0)";
    } else if (feeCat === "sc_st_ph") {
      const cat = reservationCategory.mainCategory?.toLowerCase() || "";
      const isSCST = cat === "sc" || cat === "st" || cat.includes("scheduled caste") || cat.includes("scheduled tribe");
      if (reservationCategory.isPwd === "yes") {
    ...
    const handlePayment = async () => {
      if (feePayment.paymentMode !== "online") {
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              <div className="flex justify-between">
                ...
                <span className={`px-3 py-1 text-white text-xs font-bold rounded-full flex items-center justify-center ${feePayment.paymentStatus === 'completed' ? 'bg-green-600' : feePayment.paymentStatus === 'processing' ? 'bg-amber-500' : feePayment.paymentStatus === 'failed' ? 'bg-red-500' : 'bg-primary'}`}>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label
                  className={`border-2 rounded-lg p-5 flex flex-col items-center text-center transition-all ${feePayment.paymentStatus === 'completed' || feePayment.paymentStatus === 'processing' ? 'cursor-not-allowed opacity-60' : 'cursor-pointer hover:border-primary'} ${feePayment.paymentMode === "online" ? "border-primary bg-primary/5" : "border-slate-300"}`}
                >
                  <input
                    type="radio"
                    name="payment_method"
        ...
        <div className="flex justify-end mt-6">
          {feePayment.paymentStatus === "completed" ? (
            ...
            <button
              ...
              disabled={feePayment.paymentStatus === "processing" || !feePayment.paymentMode}
              className={`h-14 px-12 text-white font-semibold rounded-xl flex items-center gap-2 transition-all ${feePayment.paymentStatus === "processing" || !feePayment.paymentMode ? "bg-slate-400 cursor-not-allowed" : "bg-primary hover:bg-primary/80"}`}
            >
              {feePayment.paymentStatus === "processing" ? (
                <>
            ...
            <div className="flex justify-between min-w-[800px] relative">
              {steps.map((step, index) => (
                <div key={step.id} className="flex-1 relative">
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center z-10 transition-all hover:opacity-80 ${index <= currentStep ? "bg-primary text-white shadow-lg" : "bg-slate-200 text-slate-500 hover:bg-slate-300"}`}
                    >
                      {index < currentStep ? (
            ...
            {currentStep < steps.length - 1 ? (
              ...
              <div className="flex gap-10">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-2 rounded-lg text-white transition-all ${!declarationConfirmed
